...
class TestEnvParser:
    ...
    def test_normalises_docker_safe_vault_placeholders(self, tmp_path: Path) -> None:
        p = tmp_path / "env"
        p.write_text(
            "A=$${vault.dev.storage.webdav.url}\nB=\\${vault.dev.storage.webdav.username}\n",
            encoding="utf-8",
        )
        d = parse_env_file(str(p))
        assert d["A"] == "${vault.dev.storage.webdav.url}"
        assert d["B"] == "${vault.dev.storage.webdav.username}"
